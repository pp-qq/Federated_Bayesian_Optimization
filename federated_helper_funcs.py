...
def acq_max(
    ...
    para_dict = {
        ...
        "all_inc_taf": all_inc_taf,
        "no_weight_learning": no_weight_learning,
    }

    if not USE_DIRECT_OPTIMIZER:
        x_tries = np.random.uniform(
            bounds[:, 0], bounds[:, 1], size=(10000, bounds.shape[0])
        )
        ...
        for x_try in x_seeds:
            res = minimize(
                lambda x: -ac(x.reshape(1, -1), para_dict),
                x_try.reshape(1, -1),
                bounds=bounds,
                method="L-BFGS-B",
            )
...
class UtilityFunction(object):
    ...
    def utility(self, x, para_dict):
        (
            ...
            para_dict["info_full_dist"],
            para_dict["info_ts"],
            para_dict["pt"],
            para_dict["ws"],
            para_dict["use_target_label"],
            para_dict["w_sample"],
            para_dict["y_max"],
            para_dict["iteration"],
            para_dict["gp_samples"],
            para_dict["all_inc_taf"],
            para_dict["no_weight_learning"],
        )
    ...
    def _rgpe(
        ...
        for n in range(N):
            ...
            var_n = obs_noise * np.squeeze(
                np.dot(np.dot(features.T, Sigma_t_inv), features)
            )
            std_n = np.sqrt(var_n)
        ...
        return optimizer_flag * (
            (post_mean - y_max - xi) * norm.cdf(z) + post_std * norm.pdf(z)
        )
    ...
    def _taf(
        ...
        if not no_weight_learning:
            ei_overall += (
                (mean - y_max - xi) * norm.cdf(z) + std * norm.pdf(z)
            ) * ws[-1]
...
class PrintLog(object):
    def __init__(self, params):
        ...
        self.sorti = sorted(
            range(len(self.params)), key=self.params.__getitem__
        )
    ...
    def print_header(self, initialization=True):
        ...
        if initialization:
            ...
            print(
                "{}Bayesian Optimization{}".format(BColours.RED, BColours.ENDC)
            )
        ...
        for index in self.sorti:
            print(
                "{0:>{1}}".format(self.params[index], self.sizes[index] + 2),
                end=" | ",
            )
        print("")
    ...
    def print_step(self, x, y, warning=False):
        ...
        if self.ymax is None or self.ymax < y:
            ...
            self.xmax = x
            print(
                "{0}{2: >10.5f}{1}".format(BColours.MAGENTA, BColours.ENDC, y),
                end=" | ",
            )
