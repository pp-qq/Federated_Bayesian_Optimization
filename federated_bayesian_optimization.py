...
import GPy
import numpy as np
from federated_helper_funcs import (
    PrintLog,
    UtilityFunction,
    acq_max,
    unique_rows,
    ...
    def maximize(self, init_points=5, n_iter=25):
        ...
        if not self.initialized:
            if self.use_init != None:
                ...
                print(
                    "Using pre-existing initializations with {0} points".format(
                        len(self.Y)
                    )
                )
            else:
        ...
        if self.fix_gp_hypers is None:
            ...
            self.gp = GPy.models.GPRegression(
                ...
                GPy.kern.RBF(
                    input_dim=self.X.shape[1],
                    lengthscale=self.fix_gp_hypers,
                    ARD=self.ARD,
                ),
            )
        ...
        if init_points > 1:
            if self.fix_gp_hypers is None:
                if self.gp_mcmc:
                    self.gp.kern.lengthscale.set_prior(
                        GPy.priors.Gamma.from_EV(1.0, 10.0)
                    )
                    self.gp.kern.variance.set_prior(
                        GPy.priors.Gamma.from_EV(1.0, 10.0)
                    )
            ...
            if self.policy == "rgpe" and not self.no_weight_learning:
                ...
                for ind_1 in np.arange(N_t):
                    ...
                    post_samples_target = gp_.posterior_samples_f(
                        self.X, full_cov=True, size=self.S
                    )
                    post_samples_target = np.squeeze(
                        post_samples_target
                    )  # shape: N_t * self.S
                    ...
                    for ind_2 in np.arange(N_t):
                        if ind_1 != ind_2:
                            for m in range(self.N):
                                ...
                                features = np.sqrt(2 / self.M) * np.cos(
                                    np.squeeze(np.dot(x_, s_rgpe.T)) + b_rgpe
                                )
                                ...
                                var_n = obs_noise_rgpe * np.squeeze(
                                    np.dot(
                                        np.dot(features.T, Sigma_t_inv),
                                        features,
                                    )
                                )
                                post_samples_m_1 = np.random.normal(
                                    mean_n, np.sqrt(var_n), self.S
                                )
                                ...
                                post_samples_m_2 = np.random.normal(
                                    mean_n, np.sqrt(var_n), self.S
                                )

                                for s in range(self.S):
                                    losses_meta[m, s] += np.bitwise_xor(
                                        self.Y[ind_1] < self.Y[ind_2],
                                        post_samples_m_1[s]
                                        < post_samples_m_2[s],
                                    )

                            for s in range(self.S):
                                losses_meta[-1, s] += np.bitwise_xor(
                                    self.Y[ind_1] < self.Y[ind_2],
                                    post_samples_target[ind_1, s]
                                    < post_samples_target[ind_2, s],
                                )

                for m in range(self.N):
                    if np.median(losses_meta[m, :]) > np.percentile(
                        losses_meta[-1, :], 95
                    ):
                        losses_meta[m, :] = 1e5
                ...
                for s in range(self.S):
                    equ_inds = np.nonzero(
                        losses_meta[:, s] == losses_meta[best_ind[s], s]
                    )[0]
                    if len(equ_inds) == 1:
            ...
            if self.policy == "taf" and not self.no_weight_learning:
                        ...
                        if ind_1 != ind_2:
                            for m in range(self.N):
                                ...
                                if mean_m_1 > mean_m_2:
                                    all_meta_features[m, count] = 1.0 / (
                                        N_t * (N_t - 1)
                                    )

                            if self.Y[ind_1] > self.Y[ind_2]:
                                target_features[count] = 1.0 / (
                                    N_t * (N_t - 1)
                                )
                ...
                for m in range(self.N):
                    dist = np.sqrt(
                        np.sum(
                            (target_features - all_meta_features[m, :]) ** 2
                        )
                    )
        ...
        if self.policy == "rgpe":
            x_max, all_ucb = acq_max(
                ac=self.util_rgpe.utility,
                gp=self.gp,
                M=self.M,
                N=self.N,
                random_features=self.random_features,
                info_full_dist=self.info_full_dist,
            ...
            if np.random.random() < self.pt[0]:
                ...
                s = np.random.multivariate_normal(
                    np.zeros(self.dim),
                    1 / (ls_target**2) * np.identity(self.dim),
                    M_target,
                )
                ...
                Sigma_t = np.dot(Phi.T, Phi) + obs_noise * np.identity(
                    M_target
                )
                Sigma_t_inv = np.linalg.inv(Sigma_t)
                nu_t = np.dot(
                    np.dot(Sigma_t_inv, Phi.T), self.Y.reshape(-1, 1)
                )
        ...
        for i in range(n_iter):
            ...
            if self.gp_model == "sklearn":
                ...
                if i >= self.gp_opt_schedule and i % self.gp_opt_schedule == 0:
                    if self.gp_mcmc:
                        ...
                        print("[Running MCMC for GP hyper-parameters]")
                        hmc = GPy.inference.mcmc.HMC(self.gp, stepsize=5e-2)
                        gp_samples = hmc.sample(num_samples=500)[
                            -300:
                        ]  # Burnin

                        gp_samples_mean = np.mean(gp_samples, axis=0)
                        print(
                            "Mean of MCMC hypers: {0}".format(gp_samples_mean)
                        )
                        ...
                        self.gp.optimize_restarts(
                            num_restarts=10, messages=False
                        )
                        self.gp_params = self.gp.parameters
                        ...
                        for ind_2 in np.arange(N_t):
                            if ind_1 != ind_2:
                                for m in range(self.N):
                                    ...
                                    x_ = np.squeeze(self.X[ind_1]).reshape(
                                        1, -1
                                    )
                                    features = np.sqrt(2 / self.M) * np.cos(
                                        np.squeeze(np.dot(x_, s_rgpe.T))
                                        + b_rgpe
                                    )
                                    features = features.reshape(-1, 1)
                                    features = features / np.sqrt(
                                    ...
                                    features = (
                                        np.sqrt(v_kernel_rgpe) * features
                                    )
                                    mean_n = np.squeeze(
                                        np.dot(features.T, nu_t)
                                    )
                                    var_n = obs_noise_rgpe * np.squeeze(
                                        np.dot(
                                            np.dot(features.T, Sigma_t_inv),
                                    ...
                                    x_ = np.squeeze(self.X[ind_2]).reshape(
                                        1, -1
                                    )
                                    features = np.sqrt(2 / self.M) * np.cos(
                                        np.squeeze(np.dot(x_, s_rgpe.T))
                ...
                if self.policy == "taf" and not self.no_weight_learning:
                    ...
                    for m in range(self.N):
                        dist = np.sqrt(
                            np.sum(
                                (target_features - all_meta_features[m, :])
                                ** 2
                            )
                        )
                        tmp = dist / self.kernel_rho
            ...
            if self.policy == "rgpe":
                ...
                if np.random.random() < self.pt[i + 1]:
                    ...
                    random_agent_n_ind = np.random.choice(
                        unused_agent_inds, 1, p=curr_ws
                    )[0]
